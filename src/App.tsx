...
const API_URL =
  'https://script.google.com/macros/s/AKfycbwFurWMyjoIhRfFQmPIVYLdKl0sfkjUbVJWPM6HLG98Cu3G4wfYhgSmEk_pUTPWHhMXgw/exec';
...
export default function App() {
  // --- Estados ---
  const [allData, setAllData] = useState<Obra[]>([]); // Almacena TODOS los datos de la hoja
  ...
  const [initialObraDetails, setInitialObraDetails] =
    useState<ObraDetails | null>(null);
  ...
  // --- Búsqueda de Empresas (Ahora Local) ---
  useEffect(() => {
    ...
    const filtered = uniqueCompanies.filter((c) =>
  ...
  useEffect(() => {
    ...
    const filtered = allComunas.filter((c) =>
      c.toLowerCase().includes(comunaInput.toLowerCase())
    );
    setComunaSuggestions(filtered.slice(0, 10));
  }, [comunaInput]);

  // --- Filtrar Obras por Empresa (Ahora Local) ---
  useEffect(() => {
    if (!selectedCompany) {
      setObras([]);
      setSelectedObraId('');
  ...
  useEffect(() => {
    ...
    const fetchDetails = async () => {
      ...
      const response = await fetch(API_URL, {
        method: 'POST',
        body: JSON.stringify({ action: 'getObraDetails', id: selectedObraId }),
      });
      const data = await response.json();
      if (data.status === 'success') {
        setObraDetails(data.data);
        setInitialObraDetails(data.data);
        setComunaInput(data.data['Comuna'] || '');
      }
      setLoading(false);
    };
    fetchDetails();
  ...
  const handleAction = async (updates: ObraDetails | {} = {}) => {
    ...
    try {
      ...
      if (data.status === 'success') {
        ...
        const currentId = selectedObraId;
        setSelectedObraId('');
        setTimeout(() => setSelectedObraId(currentId), 100);
      } else {
          ...
          <div className="form-grid-details">
            ...
            <div className="form-column">
              ...
              <div className="form-field">
                ...
                <input
                  type="text"
                  name="Tipo Construcción"
                  value={obraDetails['Tipo Construcción'] || ''}
                  onChange={handleInputChange}
                />
              </div>
              <div className="form-field">
            ...
            <div className="form-column">
              <div className="form-field">
                ...
                <select
                  name="Estado de Obra"
                  value={obraDetails['Estado de Obra'] || ''}
                  onChange={handleInputChange}
                >
                  <option value="">-- Cambiar Estado --</option>
                  {estadoObraOptions.map((e) => (
              ...
              <div className="form-field">
                <label>¿Les Vendemos?</label>
                <select
                  name="¿Les Vendemos?"
                  value={obraDetails['¿Les Vendemos?'] || ''}
                  onChange={handleInputChange}
                >
                  <option value="">-- Seleccione --</option>
                  {lesVendemosOptions.map((o) => (
              ...
              <div className="form-field">
                ...
                <textarea
                  name="Observaciones de Compra"
                  value={obraDetails['Observaciones de Compra'] || ''}
                  onChange={handleInputChange}
                  rows={2}
                ></textarea>
              </div>
              ...
              <div className="form-field">
                ...
                <input
                  ...
                  value={obraDetails['Nombre Contacto'] || ''}
                  onChange={handleInputChange}
                />
              </div>
              <div className="form-field">
                ...
                <input
                  type="text"
                  name="Cargo Contacto"
                  value={obraDetails['Cargo Contacto'] || ''}
                  onChange={handleInputChange}
                />
              </div>
            </div>
            ...
            <div className="form-column">
              <div className="form-field">
                ...
                <input
                  type="email"
                  name="Email Contacto"
                  value={obraDetails['Email Contacto'] || ''}
                  onChange={handleInputChange}
                />
              </div>
              <div className="form-field">
                ...
                <input
                  ...
                  value={obraDetails['Teléfono Contacto'] || ''}
                  onChange={handleInputChange}
                />
              </div>
              <div className="form-field">
                <label>Contacto Administrador</label>
                <input
                  type="text"
                  name="Contacto Administrador"
                  value={obraDetails['Contacto Administrador'] || ''}
                  onChange={handleInputChange}
                />
              </div>
              <div className="form-field">
                ...
                <input
                  type="text"
                  value={formatDate(obraDetails['Fecha Última Visita'])}
                  disabled
                />
              </div>
              <div className="form-field full-width">
                <label>Comentarios Última Visita</label>
                <textarea
                  name="Comentarios Última Visita"
                  value={obraDetails['Comentarios Última Visita'] || ''}
                  onChange={handleInputChange}
                  rows={4}
                ></textarea>
              </div>
